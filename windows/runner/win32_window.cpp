...
RECT CenteredWindowRect(HMONITOR monitor,
  ...
  const int left = work.left + (work_width - width) / 2;
  const int top = work.top + (work_height - height) / 2;
...
bool Win32Window::Create(const std::wstring& title,
  ...
  if (!GetCursorPos(&cursor)) {
    cursor = {static_cast<LONG>(origin.x), static_cast<LONG>(origin.y)};
  }
  HMONITOR monitor = MonitorFromPoint(cursor, MONITOR_DEFAULTTONEAREST);
  UINT dpi = FlutterDesktopGetDpiForMonitor(monitor);
  double scale_factor = dpi / 96.0;
  const RECT bounds = CenteredWindowRect(monitor, size, scale_factor);
  DWORD style = WS_OVERLAPPEDWINDOW;
  if (!resizable_) style &= ~(WS_THICKFRAME | WS_MAXIMIZEBOX);

  HWND window = CreateWindow(
      window_class, title.c_str(), style, bounds.left, bounds.top,
...
void Win32Window::SetWindowConfiguration(const Size& size,
                                         const Size& minimum_size,
                                         bool resizable) {
  minimum_width_ = minimum_size.width;
  minimum_height_ = minimum_size.height;
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    case WM_GETMINMAXINFO: {
      ...
      min_max_info->ptMinTrackSize.x = MulDiv(minimum_width_, dpi, 96);
      min_max_info->ptMinTrackSize.y = MulDiv(minimum_height_, dpi, 96);
      return 0;
    }
    case WM_DESTROY:
